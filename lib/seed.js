// Jalankan sekali di awal (otomatis dipanggil server.js kalau db.json belum ada)
// untuk membuat akun admin pertama + data contoh.
...
function seed() {
  ...
  if (db.proker.length === 0) {
    db.proker.push(
      {
        id: nextId(db, 'proker'),
        judul: 'Seminar Ulang Tahun HIMATEKKOM',
        deskripsi: 'From Passion to Profession: Embracing AI for Career Growth.',
        tag: 'seminar',
        gambar: ''
      },
      {
        id: nextId(db, 'proker'),
        judul: 'HIMATEKKOM Goes To School',
        deskripsi: 'Memperkenalkan jurusan Teknik Komputer ke siswa/i SMA & SMK.',
        tag: 'sosialisasi',
        gambar: ''
      },
      {
        id: nextId(db, 'proker'),
        judul: 'Himpunan Collab',
        deskripsi: 'Kolaborasi antar organisasi kemahasiswaan untuk tujuan bersama.',
        tag: 'kolaborasi',
        gambar: ''
      },
      {
        id: nextId(db, 'proker'),
        judul: 'Company Visit',
        deskripsi: 'Kunjungan ke perusahaan/perwakilan industri IT.',
        tag: 'kunjungan',
        gambar: ''
      }
    );
  ...
  writeDB(db);
  console.log('[seed] Database siap di data/db.json');
}
